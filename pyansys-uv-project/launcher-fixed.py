#!/usr/bin/env python3
"""
PyAnsys UV Project - Fixed Cross-platform Python Launcher
This script fixes the virtual environment path issues.
"""
...
def remove_corrupted_venv():
    ...
    if venv_path.exists():
        ...
        try:
            ...
            print_colored("Existing virtual environment removed", "green")
        except Exception as e:
            print_colored(f"Warning: Could not remove existing venv: {e}", "yellow")
    

def setup_virtual_environment():
    """Create a fresh virtual environment in the project directory."""
    venv_path = Path(".venv")
    
    # Remove any existing corrupted environment
    remove_corrupted_venv()
    
    print_colored("Creating new virtual environment in project directory...", "blue")
    result = run_command("uv venv .venv --python 3.11")
    if not result:
    ...
    if python_exe.exists():
        print_colored(f"Python executable found: {python_exe}", "green")
        return True
    else:
        print_colored(f"ERROR: Python executable not found at {python_exe}", "red")
        
        # Debug: show what's in the venv directory
        if venv_path.exists():
            print_colored("Virtual environment contents:", "blue")
...
def install_pyansys():
    ...
    print_colored("Installing PyAnsys with all dependencies...", "blue")
...
def launch_verification():
    ...
    result = run_command("uv run python main.py", check=False)
    ...
    if result and result.returncode == 0:
        ...
        print_colored("Verification had issues (this is normal for some packages)", "yellow")
        return True  # Don't fail completely for verification issues
...
def show_usage_info():
    ...
    print(f"  Virtual environment location: {current_dir}\\.venv")
...
def main():
    ...
    print("=" * 60)
    print("   PyAnsys UV Project - Environment Launcher (Fixed)")
    ...
    # Setup environment (always recreate to fix corruption)
    if not setup_virtual_environment():
        return 1
    
    # Install PyAnsys
    if not install_pyansys():
        return 1
